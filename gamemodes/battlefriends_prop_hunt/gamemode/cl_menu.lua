...
function ph_BaseMainWindow(ply, cmd, args)
	...
	function Ph:ShowAdminMenu()
		...
		Ph:CreateVGUIType("bph_use_custom_plmodel", "check", "SERVER", grid, "Enable custom models for Hunters")
		Ph:CreateVGUIType("bph_use_custom_plmodel_for_prop", "check", "SERVER", grid, "Enable custom models for Props - Make sure to enable for Hunter too.")
		Ph:CreateVGUIType("bph_customtaunts_delay", "slider", {min = 2, max = 120, init = GetConVar("bph_customtaunts_delay"):GetInt(), dec = 0, kind = "SERVER"}, grid, "Custom Taunts Delay (Seconds)")
		Ph:CreateVGUIType("bph_normal_taunt_delay", "slider", {min = 2, max = 120, init = GetConVar("bph_normal_taunt_delay"):GetInt(), dec = 0, kind = "SERVER"}, grid, "Normal Taunts Delay (Seconds)")
		Ph:CreateVGUIType("bph_autotaunt_enabled", "check", "SERVER", grid, "Enable Auto Taunt Features")
		Ph:CreateVGUIType("bph_autotaunt_delay", "slider", {min = 30, max = 180, init = GetConVar("bph_autotaunt_delay"):GetInt(), dec = 0, kind = "SERVER"}, grid, "Auto Taunts Delay (Seconds)")
		...
		Ph:CreateVGUIType("bph_prop_camera_collisions", "check", "SERVER", grid, "Enable Prop Camera collision to the wall")
		Ph:CreateVGUIType("bph_freezecam", "check", "SERVER", grid, "Enable Freecam features for team props")
		Ph:CreateVGUIType("bph_prop_collision", "check", "SERVER", grid, "Enable Prop collide on each other prop players")
		Ph:CreateVGUIType("bph_swap_teams_every_round", "check", "SERVER", grid, "Swap team every round - Disabling means team will stay forever")
		Ph:CreateVGUIType("bph_hunter_fire_penalty", "slider", 	{min = 2, max = 80, init = GetConVar("bph_hunter_fire_penalty"):GetInt(), dec = 0, kind = "SERVER"}, grid, "Hunter points penalty")
		Ph:CreateVGUIType("bph_hunter_kill_bonus", "slider", 	{min = 5, max = 100, init = GetConVar("bph_hunter_kill_bonus"):GetInt(), dec = 0, kind = "SERVER"}, grid, "Hunter kill bonus") 
		Ph:CreateVGUIType("bph_game_time", "slider", 			{min = 20, max = 300, init = GetConVar("bph_game_time"):GetInt(), dec = 0, kind = "SERVER"}, grid, "Total Game time (Minutes)") 
		Ph:CreateVGUIType("bph_hunter_blindlock_time", "slider", {min = 15, max = 60, init = GetConVar("bph_hunter_blindlock_time"):GetInt(), dec = 0, kind = "SERVER"}, grid, "Hunter blindlock time (Seconds)") 
		Ph:CreateVGUIType("bph_round_time", "slider", 			{min = 120, max = 600, init = GetConVar("bph_round_time"):GetInt(), dec = 0, kind = "SERVER"}, grid, "Game round time (Seconds)") 
		Ph:CreateVGUIType("bph_rounds_per_map", "slider", 		{min = 5, max = 30, init = GetConVar("bph_rounds_per_map"):GetInt(), dec = 0, kind = "SERVER"}, grid, "Total game Rounds per Map") 
		Ph:CreateVGUIType("bph_enable_lucky_balls", "check", "SERVER", grid, "Allow Lucky Balls Features to be spawned on breakable props (Chance is 8%)")
		...
		Ph:CreateVGUIType("bph_waitforplayers", "check", "SERVER", grid, "Wait for Players to begin the gameplay")
		Ph:CreateVGUIType("bph_min_waitforplayers", "slider", { min = 1, max = game.MaxPlayers(), init = GetConVar("bph_min_waitforplayers"):GetInt(), dec = 0, kind = "SERVER" }, grid, "Mininum Players to Wait before the game starts (default: 1)")
		...
		Ph:CreateVGUIType("", "btn", {max = 2, textdata = {
			[1] = {"--[ Current Mode is Mode: "..GetConVar('bph_enable_custom_taunts'):GetInt().." ] --", 
			function(self)
				...
				self:SetText(CusTauntConvar[GetConVar("bph_enable_custom_taunts"):GetInt()])
				local state = 0
				if GetConVar("bph_enable_custom_taunts"):GetInt() == 0 then
					state = 1
					SendTauntCommandState(1)
					self:SetText(CusTauntConvar[state])
				elseif GetConVar("bph_enable_custom_taunts"):GetInt() == 1 then
					state = 2
					SendTauntCommandState(2)
					self:SetText(CusTauntConvar[state])
				elseif GetConVar("bph_enable_custom_taunts"):GetInt() == 2 then
					state = 0
		...
		Ph:CreateVGUIType("bph_enable_plnames", "check", "SERVER", grid, "Enable Player team names to be appear on their screen.")
