...
local function MainFrame()
	if GetConVar("bph_enable_custom_taunts"):GetInt() < 1 then
	...
	local durationColumn = list:AddColumn("Duration")
	list:AddColumn("Title") -- because header is gone.
	list.m_bHideHeaders = false
	list:SetPos(10,52)
	list:SetSize(0,450)
	list:Dock(BOTTOM)
	...
	local function addTauntsToList(tbl)
		for name,val in pairs(tbl) do
			local duration = SoundDuration(val)
			local timestr = string.format("%04.1f", duration)
			list:AddLine(timestr, name)
		end
	end
	
	local function getSelectedTauntPath()
		local linename = list:GetLine(list:GetSelectedLine()):GetValue(2)
		return WHOLE_TEAM_TAUNTS[linename]
	end
	
	addTauntsToList(TEAM_TAUNTS)
...
//	end
	...
	function comb:SortAndStyle(pnl)
		...
		durationColumn:SetFixedWidth(50)
	end
		...
		elseif val == "PH:E/Custom Taunts" then
			if LocalPlayer():Team() == TEAM_PROPS then
				if PHE:GetTeamTaunt(TEAM_PROPS,true) != false then
					addTauntsToList(PHE:GetTeamTaunt(TEAM_PROPS,true))
				else
					list:AddLine("<< WARNING: NO TAUNTS DETECTED! >>")
				end
			else
				if PHE:GetTeamTaunt(TEAM_PROPS,true) != false then
					addTauntsToList(PHE:GetTeamTaunt(TEAM_HUNTERS,true))
				else
					list:AddLine("<< WARNING: NO TAUNTS DETECTED! >>")
				end
			end
	...
	local function SendToServer(snd)
		if !isplayed then
			...
			isplayed = true
			timer.Simple(GetConVar("bph_customtaunts_delay"):GetInt(), function() isplayed = false; end)
			LocalPlayer().last_taunt_time = CurTime()
		else
			chat.AddText(Color(220,40,0),"[PH:E - Taunts] Warning: ",Color(220,220,220),"You have to wait "..GetConVar("bph_customtaunts_delay"):GetInt().." seconds to play this taunt again!")
		end
	end
	
	CreateStyledButton(LEFT,86,"Play Taunt Locally",{5,5,5,5},"vgui/phehud/btn_play.vmt",FILL, function()
		if hastaunt then
			local getline = getSelectedTauntPath()
