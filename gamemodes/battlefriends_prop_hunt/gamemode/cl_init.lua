...
local function getPropEyeTrace()
	local trace = {}
	
	local lastView = LocalPlayer().lastCalculatedView
	if (lastView != nil) then
		trace.start = lastView.origin
		trace.endpos = trace.start + lastView.angles:Forward() * 300
	elseif cHullz < 24 then
...
function GM:CalcView(pl, origin, angles, fov)
	...
	if pl:Team() == TEAM_PROPS && pl:Alive() then
		if GetConVar("bph_prop_camera_collisions"):GetBool() then
			local trace = {}
...
function HUDPaint()
	-- Draw player texts
	if GetConVar("bph_enable_plnames"):GetBool() && GetConVar("ph_cl_pltext"):GetBool() --[[&& LocalPlayer():Team() != TEAM_SPECTATOR]] then
		local function drawName(pl, includeHealth)
			...
			draw.DrawText(str, "TargetIDSmall", (pl:EyePos() + addvector):ToScreen().x, (pl:EyePos() + addvector):ToScreen().y, team.GetColor(pl:Team()), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		end
		
//		for _, pl in pairs(player.GetAll()) do
//			//if pl != LocalPlayer() && (pl && pl:IsValid() && pl:Alive() && pl:Team() == LocalPlayer():Team()) then
//			if pl != LocalPlayer() && (pl && pl:IsValid() && pl:Alive() &&
//					(LocalPlayer():Team() == pl:Team() || LocalPlayer():Team() == TEAM_SPECTATOR || !LocalPlayer():Alive())
//					) then
//				local addvector = Vector(0, 0, math.Clamp(pl:EyePos():Distance(LocalPlayer():EyePos())*0.04, 16, 64))
//				-- todo: text will disappear in a specified distance.
...
//		end
		...
		for _, pl in pairs(player.GetAll()) do
			// prereqs
			if pl != LocalPlayer() && pl && pl:IsValid() && pl:Alive() && pl:Team() != TEAM_SPECTATOR then
				plTeam = pl:Team()
				
				if team == TEAM_HUNTERS then // hunters
					if team == plTeam then
						drawName(pl, true)
					end
				elseif team == TEAM_PROPS then // props
					if team == plTeam then
						drawName(pl, true)
					elseif plTeam == TEAM_HUNTERS then
						local lastView = LocalPlayer().lastCalculatedView
						if lastView != nil then
							if pl:IsLineOfSightClear(lastView.origin) then
								drawName(pl, false)
							end
						end
					end
				else // spectators (+others??)
					// TODO skip name of current spectate-ee?
					drawName(pl, true)
				end
			end
		end
	...
	if GetGlobalBool("InRound", false) then
		local blindlock_time_left = (GetConVarNumber("bph_hunter_blindlock_time") - (CurTime() - GetGlobalFloat("RoundStartTime", 0))) + 1
...
net.Receive("PH_RoundEnd_Snds", function(len)
	local snds = {}
	snds[0] = net.ReadString()
	snds[1] = net.ReadString()
	snds[2] = net.ReadString()
	
	local key = LocalPlayer():Team()
	if snds[key] == nil then
		key = 0
	end
	
	if GetConVar("ph_cl_endround_sound"):GetBool() then
		surface.PlaySound(snds[key])
	end
end)
